...
<body>
    ...
    <section class="ruler-section">
        ...
        @if($events->count() > 0)
            ...
            <div class="ruler-controls">
                <span class="zoom-label">Μέγεθος</span>
                <button class="zoom-btn" id="zoomOut" onclick="changeZoom(-1)" title="Σμίκρυνση">−</button>
                <div class="zoom-track">
                    <div class="zoom-pip active" onclick="setZoom(0)"></div>
                    <div class="zoom-pip" onclick="setZoom(1)"></div>
            ...
            <div class="ruler-track-wrapper">
                ...
                <div class="ruler-track zoom-md" id="rulerTrack">

                    @foreach($events as $index => $event)
                        <div class="ruler-event" id="event-{{ $event->id }}" data-index="{{ $index }}"
                            onclick="openPopup({{ $index }})">

                            <div class="ruler-card">
                                @if($event->image)
                                    <img class="ruler-card-img" src="{{ asset('storage/' . $event->image) }}"
                                    ...
                                    @if($event->location)
                                        <div class="ruler-card-loc">
                                            <svg width="11" height="11" viewBox="0 0 20 20" fill="none" stroke="currentColor"
                                                stroke-width="2">
                                                <path
                                                    d="M10 17.5C10 17.5 3.33333 11.1548 3.33333 8.33333C3.33333 4.65143 6.3181 1.66667 10 1.66667C13.6819 1.66667 16.6667 4.65143 16.6667 8.33333C16.6667 11.1548 10 17.5 10 17.5Z" />
            ...
            <div class="ruler-progress" id="rulerProgress">
                @foreach($events as $index => $event)
                    <div class="ruler-progress-dot {{ $index === 0 ? 'active' : '' }}" onclick="goTo({{ $index }})"
        ...
        $eventsJson = $events->map(function ($e) {
            return [
                ...
                'links' => collect($e->links ?? [])->map(function ($link) {
                    return is_array($link) ? $link : ['url' => $link, 'label' => $link];
                })->values()->toArray(),
            ];
        });
    @endphp
    ...
    <script>
        ...
        // ── Zoom
        const zoomLevels = ['zoom-sm', 'zoom-md', 'zoom-lg'];
        let zoomIndex = 1;

        function applyZoom() {
            const track = document.getElementById('rulerTrack');
            track.classList.remove(...zoomLevels);
            track.classList.add(zoomLevels[zoomIndex]);

            document.querySelectorAll('.zoom-pip').forEach((p, i) => {
                p.classList.toggle('active', i === zoomIndex);
            });
        ...
        function updateTrack() {
            ...
            cards.forEach((el, i) => el.classList.toggle('active', i === current));
        ...
        if (track) {
            track.addEventListener('mousedown', e => { isDragging = true; startX = e.clientX; startCurrent = current; });
            track.addEventListener('touchstart', e => { startX = e.touches[0].clientX; startCurrent = current; }, { passive: true });
        ...
        window.addEventListener('mousemove', e => {
            ...
            const threshold = getEventWidth() / 3;
