...
<head>
    ...
    <title>{{ __('writer.dashboard') }} - OKFN Greece</title>
    ...
    <header class="dashboard-header">
        <div class="header-container">
            <div class="header-left">
                ...
                <h1>{{ __('writer.dashboard') }}</h1>
            </div>
            <div class="header-right">
                <span class="user-name">{{ Auth::user()->name }}</span>
                <form action="{{ route('writer.logout') }}" method="POST" style="display: inline;">
                    @csrf
                    <button type="submit" class="logout-btn">{{ __('writer.logout') }}</button>
                </form>
            </div>
        </div>
    </header>
    ...
    <div class="dashboard-layout">
        ...
        <aside class="dashboard-sidebar">
            <nav class="sidebar-nav">
                <a href="{{ route('writer.dashboard') }}" class="nav-link active">
                ...
                <a href="{{ route('writer.posts.index') }}" class="nav-link">
                    <svg width="20" height="20" viewBox="0 0 20 20" fill="none">
                ...
                <a href="{{ route('writer.posts.create') }}" class="nav-link create-new">
                    <svg width="20" height="20" viewBox="0 0 20 20" fill="none">
        ...
        <main class="dashboard-main">
            ...
            <div class="stats-grid">
                <div class="stat-card">
                    ...
                    <div class="stat-content">
                        ...
                        <div class="stat-label">{{ __('writer.published_posts') }}</div>
                    </div>
                </div>
                ...
                <div class="stat-card">
                    ...
                    <div class="stat-content">
                        ...
                        <div class="stat-label">{{ __('writer.draft_posts') }}</div>
                    </div>
                </div>
                ...
                <div class="stat-card">
                    ...
                    <div class="stat-content">
                        ...
                        <div class="stat-label">{{ __('writer.total_posts') }}</div>
                    </div>
                </div>
            </div>
            ...
            <div class="posts-section">
                <div class="section-header">
                    <h2>{{ __('writer.recent_posts') }}</h2>
                    <a href="{{ route('writer.posts.create') }}" class="btn-primary">
                        <svg width="20" height="20" viewBox="0 0 20 20" fill="none">
                            <path d="M10 4.16667V15.8333" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                            <path d="M4.16667 10H15.8333" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                        </svg>
                ...
                <div class="posts-table">
                    <table>
                        <thead>
                            <tr>
                                <th>{{ __('writer.title') }}</th>
                                <th>{{ __('writer.status') }}</th>
                                <th>{{ __('writer.date') }}</th>
                                <th>{{ __('writer.views') }}</th>
                                <th>{{ __('writer.actions') }}</th>
                            </tr>
                        </thead>
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="action-buttons">
                                        <a href="{{ route('writer.posts.edit', $post) }}" class="btn-icon" title="{{ __('writer.edit') }}">
                                        ...
                                        @if($post->status === 'published')
                                            <a href="{{ route('posts.show', $post->slug) }}" class="btn-icon" target="_blank" title="{{ __('writer.view') }}">
                                                <svg width="18" height="18" viewBox="0 0 18 18" fill="none">
                                        ...
                                        @endif
                                        <form action="{{ route('writer.posts.destroy', $post) }}" method="POST" style="display: inline;" onsubmit="return confirm('{{ __('writer.confirm_delete') }}')">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="btn-icon btn-delete" title="{{ __('writer.delete') }}">
                                                <svg width="18" height="18" viewBox="0 0 18 18" fill="none">
                ...
                <div class="empty-state">
                    ...
                    <h3>{{ __('writer.no_posts') }}</h3>
                    <p>{{ __('writer.no_posts_description') }}</p>
                    <a href="{{ route('writer.posts.create') }}" class="btn-primary">
                        {{ __('writer.create_first_post') }}
                    </a>
                </div>
                @endif
            </div>
