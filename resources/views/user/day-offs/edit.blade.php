...
<head>
    ...
    <title>Επεξεργασία Άδειας - {{ Auth::user()->name }}</title>
    <link href="https://fonts.googleapis.com/css2?family=Roboto:wght@300;400;500;700;900&display=swap" rel="stylesheet">
    <link rel="stylesheet" href="{{ asset('css/writerdashboard.css') }}">
    <link rel="stylesheet" href="{{ asset('css/userPanel/dayoff-edit.css') }}">
    ...
    <div class="dashboard-layout">
        ...
        <main class="dashboard-main">
            <div class="page-header">
                <h2>Επεξεργασία Άδειας</h2>
                <p class="page-subtitle">Ενημερώστε τα στοιχεία της άδειάς σας</p>
            </div>

            <div class="form-container">
                <form
                    action="{{ route('user.day-offs.update', ['locale' => app()->getLocale(), 'id' => $dayOff->id]) }}"
                    method="POST" id="dayOffForm">
                    @csrf
                    @method('PUT')

                    <!-- Calculated Days Display -->
                    <div class="days-display" id="daysDisplay">
                        <div class="days-number" id="daysNumber">{{ $dayOff->total_days }}</div>
                        <div class="days-label">Ημέρες Άδειας</div>
                    </div>
                    ...
                    <div class="form-row">
                        <div class="form-group">
                            ...
                            <input type="date" id="start_date" name="start_date"
                                ...
                                value="{{ old('start_date', $dayOff->start_date->format('Y-m-d')) }}" required>
                            @error('start_date')
                        ...
                        <div class="form-group">
                            ...
                            <input type="date" id="end_date" name="end_date"
                                ...
                                value="{{ old('end_date', $dayOff->end_date->format('Y-m-d')) }}" required>
                            @error('end_date')
                                <span class="error-message">{{ $message }}</span>
                            @enderror
                        </div>
                    ...
                    <div class="form-group">
                        ...
                        <textarea id="reason" name="reason" class="form-input @error('reason') error @enderror"
                    ...
                    <div class="form-group">
                        ...
                        <textarea id="notes" name="notes" class="form-input @error('notes') error @enderror"
                            placeholder="Προσθέστε οποιεσδήποτε πρόσθετες σημειώσεις...">{{ old('notes', $dayOff->notes) }}</textarea>
                        <span class="help-text">Προαιρετικό - Οποιαδήποτε επιπλέον πληροφορία που θέλετε να
                            μοιραστείτε</span>
                        @error('notes')
                            <span class="error-message">{{ $message }}</span>
                        @enderror
                    ...
                    <div class="form-actions">
                        <a href="{{ route('user.day-offs.index', ['locale' => app()->getLocale()]) }}"
                            class="btn-cancel">
                            Ακύρωση
                        </a>
